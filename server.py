...
# ============================================================
# x402 PAYMENT CONFIGURATION — СТРОГО
# ============================================================
PAYMENT_CONFIG = {
    "x402Version": 2,
    "resource": {
        "url": "https://crypto-snapshot-pro.onrender.com",
        "description": "Real-time crypto market analysis using 8-factor scoring: RSI, EMA(20/50), Volume Ratio, Bollinger Bands, RSI Divergence, ATR volatility, Pivot Points. Outputs: LONG/SHORT/HOLD signal, conviction level (LOW/MEDIUM/HIGH/VERY HIGH), Entry/Target/Stop levels, Risk/Reward ratio. Supports 500+ Binance pairs. Price: $0.025 per request.",
...
def create_402_response():
    ...
    return Response(
        ...
        status_code=402,
        headers={"payment-required": encoded, "content-type": "text/plain"}
    )


def has_valid_x402_payment(request: Request) -> bool:
    """Проверка наличия x402 платежа"""
    h = request.headers
    return bool(h.get("x-payment") or h.get("payment-signature") or h.get("authorization"))


# ============================================================
# Технические функции анализа
# ============================================================
...
def get_signal_from_factors(rsi, price_ema20, price_ema50, volume_ratio, high_low_range, macd, macd_signal, macd_hist, bb_upper, bb_middle, bb_lower, rsi_divergence, pivot):
    long_score = short_score = 0
    if rsi < 30: long_score += 2
    elif rsi > 70: short_score += 2
    elif rsi < 40: long_score += 1
    elif rsi > 60: short_score += 1
    if price_ema20 > price_ema50: long_score += 1
    else: short_score += 1
...
async def fetch_ticker(symbol: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if response.status_code != 200:
            raise HTTPException(status_code=400, detail=f"Binance API error")
        data = response.json()
...
async def payable_endpoint(request: Request):
    if not has_valid_x402_payment(request):
...
async def crypto_snapshot(request: Request):
    ...
    if request.method == "POST":
        try:
            ...
            if "symbol" in body:
                ...
                symbol = body["message"].get("content", "")
        except:
            pass
    else:
    ...
    if not symbol:
        return AgentResponse(message={"role": "assistant", "content": "Send a symbol like BTC, ETH, SOL"})
    ...
    try:
        ...
        signal, signal_desc, long_score, short_score = get_signal_from_factors(
            rsi, ema20, ema50, volume_ratio, high_low_range, macd, macd_signal, macd_hist,
            bb_upper, bb_middle, bb_lower, rsi_divergence, pivot
        )
...
• RSI(14): {rsi:.1f}
• EMA20: {format_price(ema20)}
• EMA50: {format_price(ema50)}
🎯 Entry: {format_price(entry)} | Target: {format_price(target)} | Stop: {format_price(stop)}
"""
...
@app.get("/")
async def root():
    return {"service": "Crypto Snapshot Pro", "x402": True, "agentId": "3613"}
