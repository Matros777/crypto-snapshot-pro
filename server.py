...
def has_payment(request: Request):
    headers = request.headers
    return bool(headers.get("x-payment") or headers.get("payment-signature") or headers.get("authorization"))
...
def calculate_bollinger_bands(closes: list[float], period: int = 20, std_dev: float = 2) -> tuple:
...
def get_signal_from_factors(rsi: float, ema20: float, ema50: float,
                           ...
                           rsi_divergence: str, pivot: dict) -> tuple:
...
async def crypto_snapshot(request: Request):
    ...
    try:
        ...
        # MACD упрощённо (для совместимости)
